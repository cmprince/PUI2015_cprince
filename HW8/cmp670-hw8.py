...
def myLabeledColorBar(axes, bounds, 
    ...
    """Generate a stacked bar alternative to a pie chart.
    
    Keyword arguments:
    axes -- matplotlib axes object in which to draw color bar
    bounds -- list of numerical values for each component to be plotted
    textlabs -- list of labels for each component (default is empty)
    colorlist -- a list of colors for each component (default is 'gray')
    drawedges -- boolean, true if edges should be drawn (default is True)
    orientation -- string 'horizontal' or 'vertical', direction to draw boxes
    """
    ...
    cb = mpl.colorbar.ColorbarBase(axes, cmap=cmap, norm=norm, 
                                   boundaries=bounds,
                                   drawedges=drawedges,
                                   spacing='proportional',
                                   orientation=orientation)

    #Draw labels along the x-axis if the colorbar is horizontal, otherwise
    #draw them on the y-axis.
    #Draw the labels in the centers of the regions set by the boundaries.
    if (orientation == 'horizontal'):
        cb.ax.get_xaxis().set_ticks([])
        for j in range(len(bounds[1:])):
            xpos = ((bounds[j+1]-bounds[j])/2 + bounds[j])/float(bounds[-1])
            xlab = textlabs[j % len(textlabs)] + '\n' + \
                   '{0:g}'.format((bounds[j+1]-bounds[j]))
            cb.ax.text(xpos, .5, xlab, ha='center', va='center')
                       #bbox=dict(facecolor='white', ec='none', alpha=0.65))

    else:
        cb.ax.get_yaxis().set_ticks([])
        for j in range(len(bounds[1:])):
            ypos = ((bounds[j+1]-bounds[j])/2 + bounds[j])/float(bounds[-1])
            ylab = textlabs[j % len(textlabs)] + '\n' + \
                     '{0:g}'.format((bounds[j+1]-bounds[j]))
            cb.ax.text(.5, ypos, ylab, ha='center', va='center')
                       #bbox=dict(facecolor='white', ec='none', alpha=0.65))
    
    return cb

def addCBar(im, ax, ticks=5, side='top', size='5%', pad=0.03):
    """Place a colorbar on an arbitrary side of an image plot.
    
    Keyword arguments:
    im -- image plot from which to obtain colors
    ax -- axes in which to draw the colorbar (should match axes of im)
    ticks -- number of ticks in the colorbar (default = 5)
    side -- string 'top', 'bottom', 'left' or 'right'; side of plot on which
            to draw the colorbar (default = 'top')
    size -- amount of the axes range to steal for the colorbar 
            (default = '5%')
    pad -- gap to leave between image and bar (default = 0.03)
    """
    ...
    #Create the colorbar from data im and set tick locations using side
...
#for graph titling (so change this for a different data set)
dataperiod = 'July 2014'
cbdata=pd.read_csv(fname, parse_dates=['starttime'])
...
menhours = [cbdata['starttime'][h].hour for h in range(len(cbdata)) \
            if cbdata['gender'][h] == 1] 
mendays = [cbdata['starttime'][d].day for d in range(len(cbdata)) \
           if cbdata['gender'][d] == 1]
womenhours = [cbdata['starttime'][h].hour for h in range(len(cbdata)) \
              if cbdata['gender'][h] == 2] 
womendays = [cbdata['starttime'][d].day for d in range(len(cbdata)) \
             if cbdata['gender'][d] == 2]
unknownhours = [cbdata['starttime'][h].hour for h in range(len(cbdata)) \
                if cbdata['gender'][h] == 0]
custhours = [cbdata['starttime'][h].hour for h in range(len(cbdata)) \
             if cbdata['usertype'][h] == 'Customer'] 
custdays = [cbdata['starttime'][d].day for d in range(len(cbdata)) \
            if cbdata['usertype'][d] == 'Customer']
subschours = [cbdata['starttime'][h].hour for h in range(len(cbdata)) \
              if cbdata['usertype'][h] == 'Subscriber'] 
subscdays = [cbdata['starttime'][d].day for d in range(len(cbdata)) \
             if cbdata['usertype'][d] == 'Subscriber']
...
print 'Read n=' + str(len(cbdata)) + ' records from data set ' + fname + \
      '. The average tripduration is ' + \
      '{0:.2f}'.format(avTripduration) + ' seconds.'
...
pl.title('Total Citi Bike rides in ' + dataperiod + \
         ' by hour, per gender and usertype')
...
cb2 = myLabeledColorBar(ax6, bounds1, lablist1, collist1, 
                        orientation='vertical')

collist2 = ['orange', 'pink']
lablist2 = ['Subscribers','Customers']
bounds2 = [0, totalSubsc, totalCust + totalSubsc]
cb3 = myLabeledColorBar(ax7, bounds2, lablist2, collist2, 
                        orientation='vertical')
